...
bool  bdResetErrors()   { bdERR_CODE = 0; return true; }

string strofchar(string C, int n) {
	string RES = "";

	for (int i = 0; i < n; i++) RES.append(C);
...
void bdouble::sscan(string s) {
    ...
    for (i = z + 1; i < n; ++ i) a[DoubleSize - (i - (z + 1)) - 1] = s[i] - '0';
    for (i = z - 1; i >= 0; -- i) a.push_back(s[i] - '0');
}

void bdouble::normal() {
    while (a.sz < DoubleSize + 1) a.push_back (0);
}
